...
export class AssetService {
  ...
  list(params: { page: number; pageSize: number; keyword?: string; type?: string; department?: string; status?: string; location?: string; sortBy?: string; sortOrder?: string }): { items: AssetRow[]; total: number } {
  ...
  create(data: Record<string, unknown>, userId: string, username: string, ip: string): AssetRow {
    ...
    if (!assetCode) {
      // 自动生成：根据部门匹配前缀+后缀，未匹配则使用默认
      const department = String(data.department || '');
      const prefixRow = this.db.get('SELECT prefix, suffix FROM code_prefixes WHERE department = ?', [department]) as Record<string, unknown> | undefined;
      const prefix = prefixRow ? String(prefixRow.prefix) : 'ZC';
      const suffix = prefixRow ? String(prefixRow.suffix || '') : '';
    ...
    this.db.run(
      `INSERT INTO assets (id, asset_code, name, type, model, department, "user", purchase_date, status, location, remark, wired_macs, wireless_macs, hostnames, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [id, assetCode, data.name || '', data.type || '其他', data.model || '', data.department || '', data.user || '', data.purchaseDate || '', data.status || '在用', data.location || '', data.remark || '', wiredMacs, wirelessMacs, hostnames, now, now]
    );
  ...
  update(id: string, data: Record<string, unknown>, userId: string, username: string, ip: string): AssetRow | null {
    ...
    this.db.run(
      `UPDATE assets SET name = ?, type = ?, model = ?, department = ?, "user" = ?, purchase_date = ?, status = ?, location = ?, remark = ?, wired_macs = ?, wireless_macs = ?, hostnames = ?, updated_at = ? WHERE id = ?`,
      [String(data.name ?? existing.name), String(data.type ?? existing.type), String(data.model ?? existing.model), String(data.department ?? existing.department), String(data.user ?? existing.user), String(data.purchaseDate ?? existing.purchase_date), String(data.status ?? existing.status), String(data.location ?? existing.location), String(data.remark ?? existing.remark), wiredMacs, wirelessMacs, hostnames, now, id]
    );
      ...
      for (let i = 0; i < dataList.length; i++) {
        ...
        try {
          ...
          if (!assetCode) {
            ...
            const pRow = this.db.get('SELECT prefix, suffix FROM code_prefixes WHERE department = ?', [dept]) as Record<string, unknown> | undefined;
            const prefix = pRow ? String(pRow.prefix) : 'ZC';
            const suffix = pRow ? String(pRow.suffix || '') : '';
            assetCode = this.codePrefixService.generateNextCode(prefix, suffix);
          } else {
          ...
          const wirelessMacs = stringifyArray(data.wirelessMacs);
          const hostnames = stringifyArray(data.hostnames);
          this.db.run(`INSERT INTO assets (id, asset_code, name, type, model, department, "user", purchase_date, status, location, remark, wired_macs, wireless_macs, hostnames, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
            [id, assetCode, String(data.name), String(data.type), String(data.model || ''), String(data.department || ''), String(data.user || ''), String(data.purchaseDate || ''), String(data.status), String(data.location || ''), String(data.remark || ''), wiredMacs, wirelessMacs, hostnames, now, now]);
  ...
  listAll(params?: { keyword?: string; type?: string; department?: string; status?: string; location?: string; sortBy?: string; sortOrder?: string }): AssetRow[] {
    const conditions: string[] = [];
    const values: unknown[] = [];
    if (params?.keyword) { conditions.push('(name LIKE ? OR asset_code LIKE ? OR "user" LIKE ? OR remark LIKE ? OR wired_macs LIKE ? OR wireless_macs LIKE ? OR hostnames LIKE ?)'); const kw = `%${params.keyword}%`; values.push(kw, kw, kw, kw, kw, kw, kw); }
    if (params?.type) { conditions.push('type = ?'); values.push(params.type); }
