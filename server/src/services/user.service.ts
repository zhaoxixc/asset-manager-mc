...
export class UserService {
  ...
  create(data: { username: string; password: string; realName: string; role: string; status: string }, userId: string, username: string, ip: string): Record<string, unknown> | { error: string } {
    ...
    this.db.run('INSERT INTO users (id, username, password, real_name, role, status, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)', [id, data.username, hashedPassword, data.realName, data.role, data.status || 'active', now, now]);
  ...
  update(id: string, data: { realName?: string; role?: string; status?: string; password?: string }, userId: string, username: string, ip: string): Record<string, unknown> | null {
    const existing = this.getById(id);
    if (!existing) return null;
    const now = getNow();
    if (data.password) {
      const hashedPassword = hashPassword(data.password);
      this.db.run('UPDATE users SET real_name = ?, role = ?, status = ?, password = ?, updated_at = ? WHERE id = ?', [data.realName ?? existing.realName, data.role ?? existing.role, data.status ?? existing.status, hashedPassword, now, id]);
    } else {
      this.db.run('UPDATE users SET real_name = ?, role = ?, status = ?, updated_at = ? WHERE id = ?', [data.realName ?? existing.realName, data.role ?? existing.role, data.status ?? existing.status, now, id]);
    }
